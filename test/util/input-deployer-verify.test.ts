...
import {ENV, ISSUE_LINK_TYPE_NAME, VERIFY_STATUS, REASON} from '../../src/util/constants'
import {previousEnv} from '../../src/util/util-functions'
import Verifier from '../../src/util/input-deployer-verify'
import {JiraMock, RFCWORKFLOW, RFDWORKFLOW} from './jira-mock'
...
function getDefaultRfdIssueInfo(issueKey: any) {
  const rfdInfoStub: any = {
    fields: {
      customfield_10121: {
        value: '',
      },
...
describe('RFC Verification:', function () {
  ...
  for (const env of environments) {
    for (const status of RFCWORKFLOW.ALL_STATUS) {
      it(`env: ${env}, status: ${status.name} (${status.id}), no RFDs`, async function (this: Context) {
        ...
        await expect(result).toMatchSnapshot(this as any, `388a19ef-a1d7-47ab-9ca4-4eff419aab96-${env}-${status.id}`)
  ...
  it("When RFC/RFD conditions are not met, return result with 'Not Ready' status", async function () {
    ...
    // Act
    const result = await verifier.verifyBeforeDeployment()
...
describe('obtainCurrentRfcRfdContext:', function () {
  ...
  context('Function isReadyForDeployment()', function () {
    ...
    it("When RFDs blockedBy others, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_RFD_BLOCKED", async function () {
      ...
      const stubObtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').returns(Promise.resolve(blockedByRfcRfdContext))
      ...
      // Verify
      stubObtainCurrentRfcRfdContext.calledWith(env, rfcIssueKey)
      expect(result.status).toEqual(VERIFY_STATUS.NOT_READY)
      ...
      blocked.issueItems.forEach((item: any) => {
    ...
    it("Some of current stage RFDs are not approved, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_RFD_NOT_APPROVED", async function () {
      ...
      const stubObtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(rfdNotApprovedRfcRfdContext)
      ...
      const resultIssueItemsIssueKeys = notApproved.issueItems.map((item: any) => item.issueKey)
      expect(resultIssueItemsIssueKeys).toContain(rfdNotApprovedRfcRfdContext.rfdsByEnv[env].rfds[1].issueKey)
      notApproved.issueItems.forEach((issueItem: any) => {
    ...
    it("Some of previous stage RFDs are not closed, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_PREVIOUS_RFD_NOT_CLOSED", async function () {
      ...
      const stubobtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(previousRfdNotClosedRfcRfdContext)
      ...
      const resultIssueItemsIssueKeys = pNotClosed.issueItems.map((item: any) => item.issueKey)
      expect(resultIssueItemsIssueKeys).toContain(
        previousRfdNotClosedRfcRfdContext.rfdsByEnv[env].previousEnvRfds[0].issueKey
      )
      pNotClosed.issueItems.forEach((issueItem: any) => {
    ...
    it("ENV=dlvr but RFC is not Authorized to Int, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_RFC_NOT_AUTHORIZED", async function () {
      ...
      const stubObtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(dlvrRFCnotAuthorizedRfcRfdContext)
      ...
      expect(notAuthorized.issueItems[0].status).not.toEqual(RFCWORKFLOW.STATUS_APPROVED)
    ...
    it("ENV=test but RFC is not Authorized to Test, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_RFC_NOT_AUTHORIZED", async function () {
      ...
      const stubobtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(testRFCnotAuthorizedRfcRfdContext)
    ...
    it("ENV=prod but RFC is not Authorized to Prod, return result: status='Not Ready', rfcRfdContext and reason with code: REASON_CODE_RFC_NOT_AUTHORIZED", async function () {
      ...
      const stubObtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(prodRFCnotAuthorizedRfcRfdContext)
    ...
    it("When all conditions passed verification, return result: status='Ready' and rfcRfdContext", async function () {
      ...
      const stubObtainCurrentRfcRfdContext = sandbox.stub(verifier, 'obtainCurrentRfcRfdContext').resolves(testAllVerifiedRfcRfdContext)
  ...
  context('Function obtainCurrentRfcRfdContext()', function () {
    it("At 'dlvr' env, With rfc/rfd issues can be found, returns Rfc/Rfd context for caller function", async function () {
      ...
      .filter((link: any) => link.type.name === ISSUE_LINK_TYPE_NAME.RFC_FRD)
      .reduce((accumulator: any, _link: any) => {
      ...
      dlvrRfcRfdContext.rfdsByEnv[env].rfds.forEach((rfdInfo: any) => {
        ...
        if (rfdInfo.blockedBy) {
          rfdInfo.blockedBy.forEach((blockedByIssue: any) => {
      ...
      expect(dlvrRfcRfdContext.rfdsByEnv[env].previousEnvRfds.every((p: any) => p.env === previousEnv(env))).toBe(true)
    })
    ...
    it("At 'test' env, With rfc/rfd issues can be found, returns Rfc/Rfd context for caller function", async function () {
      ...
      expect(testRfcRfdContext.rfdsByEnv[env].rfds.map((rfd: any) => rfd.issueKey)).toEqual(
        expect.arrayContaining(['RFD-AUTO-TEST-01', 'RFD-BUSINESS-TEST-01'])
      )
      testRfcRfdContext.rfdsByEnv[env].rfds.forEach((rfdInfo: any) => {
        expect(rfdInfo.blockedBy).toBeInstanceOf(Array)
        expect(rfdInfo.env).toEqual(env)
        if (rfdInfo.blockedBy) {
          rfdInfo.blockedBy.forEach((blockedByIssue: any) => {
      ...
      expect(testRfcRfdContext.rfdsByEnv[env].previousEnvRfds.every((p: any) => p.env === previousEnv(env))).toBe(true)
    })
